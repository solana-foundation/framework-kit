...
vi.mock('@wallet-standard/app', () => {
	...
	return {
		...
		// helper to update the mocked wallet list per test
...
describe('Wallet Standard connector metadata', () => {
	beforeEach(() => {
		// Ensure window exists so ready resolves to true.
		(globalThis as Record<string, unknown>).window = {};
	});

	it('sets a stable id, kind, and ready flag on connectors', async () => {
	...
	it('deduplicates connectors by id when discovering wallets', async () => {
